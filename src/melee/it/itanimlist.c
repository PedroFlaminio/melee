...
void it_802790C0(Item_GObj* item_gobj, CommandInfo* cmd)
{
    ...
    hit->x40_b4 = CMD_U8(cmd->u, 0);
    hit->x41_b4 = (CMD_U8(cmd->u, 1) >> 7) & 1;
    hit->x41_b5 = (CMD_U8(cmd->u, 1) >> 6) & 1;
    hit->x41_b6 = (CMD_U8(cmd->u, 1) >> 5) & 1;
    hit->x41_b7 = (CMD_U8(cmd->u, 1) >> 4) & 1;
    hit->x42_b0 = (CMD_U8(cmd->u, 1) >> 3) & 1;
    hit->x42_b1 = (CMD_U8(cmd->u, 1) >> 2) & 1;
    hit->x42_b2 = (CMD_U8(cmd->u, 1) >> 1) & 1;
    hit->x42_b3 = CMD_U8(cmd->u, 1) & 1;
    hit->x42_b4 = (CMD_U8(cmd->u, 2) >> 7) & 1;
    hit->x42_b5 = (CMD_U8(cmd->u, 2) >> 6) & 1;
    hit->x42_b6 = (CMD_U8(cmd->u, 2) >> 5) & 1;
    hit->x42_b7 = (CMD_U8(cmd->u, 2) >> 4) & 1;
    hit->x43_b0 = (CMD_U8(cmd->u, 2) >> 3) & 1;
    hb->x138 = (CMD_U8(cmd->u, 2) >> 2) & 1;
    ++cmd->u;
