...
/* Direct vertex writes.  The console stores each component in the FIFO
 * register; the host hands the same bytes, in the same order and width, to
 * its GX recorder, which reads them by the current vertex descriptor. */
#ifdef MELEE_HOST
#define PS_FIFO_F32(v) melee_host_gx_submit_f32((f32) (v))
#define PS_FIFO_U8(v) melee_host_gx_submit_u8((u8) (v))
#else
            ...
            if (up_len != 0.0f) {
                ...
                for (; primitive_count != 0; primitive_count--) {
                    ...
                    for (i = count; i > 0; i--) {
                        ...
                        PS_FIFO_F32(up.x * tx + (right.x * sx + x));
                        PS_FIFO_F32(up.y * tx + (right.y * sx + y));
                        PS_FIFO_F32(up_z * tx + (right_z * sx + z));
...
static inline void psDispSubAppSRT(HSD_Particle* pp, u8* texform)
{
    ...
    if (pp->kind & Trail) {
        ...
        PS_FIFO_F32(-ax + cur_x);
        PS_FIFO_F32(-ay + cur_y);
        PS_FIFO_F32(cur_z);
        if (pp->kind & DispTexture) {
            PS_FIFO_U8((pp->kind >> 16) & 0xC);
        }
        PS_FIFO_F32(-bx + cur_x);
        PS_FIFO_F32(-by + cur_y);
        PS_FIFO_F32(cur_z);
        if (pp->kind & DispTexture) {
            PS_FIFO_U8(((pp->kind >> 16) & 0xC) + 1);
        }
